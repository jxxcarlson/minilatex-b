            ...
            , test "parse macro" <|
                \_ ->
                    "\\strong{stuff}"
                        ...
                        |> run (expression 0)
                        |> Expect.equal (Ok (Macro "strong" Nothing [ Text "stuff" { chunkOffset = 0, content = "stuff", length = 13, offset = 0 } ] { chunkOffset = 0, content = "\\strong{stuff}", length = 14, offset = 0 }))
            ...
            , test "parse arg" <|
                \_ ->
                    "{123}"
                        ...
                        |> run (arg 0)
                        |> Expect.equal (Ok (Text "123" { chunkOffset = 0, content = "123", length = 4, offset = 0 }))
            , test "parse macro name" <|
                \_ ->
                    "\\strong"
                        |> run (macroName 0)
                        |> Expect.equal (Ok ( "strong", { chunkOffset = 0, content = "strong", length = 7, offset = 0 } ))
